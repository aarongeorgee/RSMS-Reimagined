...
export default function AcademicIntelligence({page,token,role}:{page:PageName,token:string|null,role:string}){
  ...
  if(page==='Overview'){
    ...
    return <>
      ...
      <div className="overview-top smart-overview-top">
        ...
        <section className="stat-card"><div className="stat-label">Overall attendance <ShieldCheck size={18}/></div><div className="big-number">{summary?.overallPercentage??0}<span>%</span></div><p>{summary?.attended??0} of {summary?.total??0} reconstructed + recorded classes attended</p><Progress value={summary?.overallPercentage??0}/><button className="warning-link">Internal threshold 80% · ESE threshold 75%</button></section>
  ...
  if(page==='Attendance'){
    return <>
      <div className="summary-band"><div><span>Official snapshot</span><h2>12 Sep</h2></div><div><span>Reconstructed through</span><h2>22 Sep</h2></div><div><span>Internal / ESE</span><h2>80% / 75%</h2></div><div><span>Current classes</span><h2>{summary?.total??0}</h2><small className="data-freshness">{syncedLabel?`Updated ${syncedLabel}`:'Waiting for sync'}</small></div></div>
